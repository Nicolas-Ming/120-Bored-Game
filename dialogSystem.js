...
dialogSystem = function(game){
	...
	this.NEXT_TEXT = '[Press A     Press B]';	// text to display for next prompt
	this.NEXT_X = 775;			// next text prompt x-position
	this.NEXT_Y = 574;
		// next text prompt y-position
...
dialogSystem.prototype = {
	...
	update: function() {
		...
		if(this.input.keyboard.justPressed(Phaser.Keyboard.A) && !this.dialogTyping) {
			// trigger dialog
			console.log('A');
			this.TypeText();
		}else if(this.input.keyboard.justPressed(Phaser.Keyboard.B) && !this.dialogTyping) {
			// trigger dialog
			console.log('B');
			this.TypeB();
		}
